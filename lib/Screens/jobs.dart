...
class _JobsState extends State<Jobs> {
  ...
  List<NationalJob> nationaljobs = getNationalJobs();
  ...
  List<Widget> buildNationalJobs(){
    List<Widget> list = [];
    list.add(SizedBox(width: 32,));
    for (var i = 0; i < nationaljobs.length; i++) {
      list.add(buildNationalJob(nationaljobs[i]));
    }
    list.add(SizedBox(width: 16,));
    return list;
  }

  Widget buildNationalJob(NationalJob job){
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => NationalDetail(job: job)),
        );
      },
      child: Container(
        width: 200,
  ...
  List<Widget> buildNationalLast(){
    List<Widget> list = [];
    for (var i = nationaljobs.length - 1; i > -1; i--) {
      list.add(buildNationalLasts(nationaljobs[i]));
    }
    return list;
  }

  Widget buildNationalLasts(NationalJob job){
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
